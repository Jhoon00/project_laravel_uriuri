<x-app-layout>
    ...
    <div class="min-h-full mt-10">
        <div class="flex justify-center mb-4">
            <img src="{{ asset('storage/' .$post->image) }}"/ class="w-56 h-56">
        </div>
        <form action="/post/{{$post->id}}" method="POST" enctype="multipart/form-data">
            ...
            <div s x-data="{ images: [] }">
                <div class="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-xl max-w-2xl bg-white">
                    <input class="title border-t-0 border-x-0 border-gray-300 p-2 mb-4 outline-none" spellcheck="false" placeholder="{{$post->title}}" type="text" name="title">
                    <input class="title border-t-0 border-x-0 border-gray-300 p-2 mb-4 outline-none" spellcheck="false" placeholder="{{$post->price}}" type="number" name="price">
                    <textarea class="description bg-transparent sec p-3 h-60 border border-gray-300 outline-none" spellcheck="false" placeholder="{{$post->content}}" name="content"></textarea>
                    ...
                    <div class="buttons flex justify-end">
                        <button type="submit" class=" border border-green-500 p-1 px-4 font-semibold cursor-pointer text-gray-200 ml-2 bg-orange-400">Update</button>
                    </div>
                </div>
            </div>
        </form>
